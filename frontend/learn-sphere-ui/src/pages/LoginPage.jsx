...
const LoginPage = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (normalized === "instructor@example.com" && password === "Instructor@123") {
        const adminData = {
        ...
        localStorage.setItem(
          "learnsphere_user",
          JSON.stringify({ name: adminData.name, email: adminData.email, role: adminData.role })
        );
        localStorage.setItem("studentName", adminData.name);
        localStorage.setItem("studentEmail", adminData.email);
